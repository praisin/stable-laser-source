function [avar]=allan(data, tau, name)
...
n=length(data.freq);
jj=length(tau);
m=floor(tau*data.rate); % number of points in the particular interval, NOT the integration time
...
tic;  % start timer, Matlabs way of measuring performance


% in all of the followin calculations, the actual time is not of importance
% but only the m. 
for j=1:jj   % iterate over each time interval of interest
        
    D=zeros(1,n-m(j)+1);  % amount of bins
   
    D(1)=sum(data.freq(1:m(j)))/m(j); % first element computed manually
   
    % calculate the averages
    for i=2:n-m(j)+1 % average over each bin
        % To calculate the mean of adjacent clusters,
        % take the last mean, add the next element and remove the first to
        % get the new mean. 
    ...
    % classical standard deviation of the averaged bins. NO OVERLAP! Plus
    % the last sample is not included in bot [1,2,3] [4,5,6] not
    % [1,2,[3],4,5] with one value being in two different places.
    avar.sig(j)=std(D(1:m(j):n-m(j)+1));  
    avar.sigerr(j)=avar.sig(j)/sqrt(n/m(j));
    
    %normal Allan deviation 
    avar.sig2(j)=sqrt(0.5*mean((diff(D(1:m(j):n-m(j)+1)).^2)));  % just like in the definition. Note the use of diff for efficiency & clean code
    avar.sig2err(j)=avar.sig2(j)/sqrt(n/m(j));
    
    %overlapping Allan deviation (when to use it?)
    % (commented out at 1.8.2016 by P. Raisin)
%     z1=D(m(j)+1:n+1-m(j));  % z1=D(m(j)+1:end)  all elements excpet for the first m(j)
%     z2=D(1:n+1-2*m(j));     % z2=D(1:(n+1-m(j))-m(j)) all elements except the last m(j)
%     u=sum((z1-z2).^2);      % take the difference and square
%     avar.osig(j)=sqrt(u/(n+1-2*m(j))/2);    % normalize: we have n+1-2*m(j) elements so this is the value to normalize with
